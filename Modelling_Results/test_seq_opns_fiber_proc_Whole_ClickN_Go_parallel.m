%%  Fibre computation and evaluation algorithm
% This script does the following tasks
%  1) Loads 3D stack of rgb images and preprocesses the Red channel using the novel contrast stabilisation filter. This is to stabilise the slice to slice fluctuations.
%  2) Smoothing and preparing the data (Gaussian filter) for structure tensor computation (Local scale= 1 ; integration scale=5)
%  3) Structure tensor computation. This is parallelised for efficient computation.
%  4) Computation of the projection angles (X,Y and Z planes). Later, these will be used for evaluation of vectors computed by Structure tensor.
%  5) Making the hsv model of orientation and saving the resulting images as tifs
% Finally, The code is prepared for use with nohup and run in hpc (Everything in Single Click and GO)
...
cd /hpc_atog/btho733/ABI/pacedSheep01/Dataset_Scale4/After_Amira/rgbseg_outputpng;
...
offset=300;
for i=1:len
infile=sprintf('rgb_%05d.png',i+offset);%outfile=sprintf('cut_%05d.tif',i-500);                       %change this for new image
im=imread(infile);
% im=im(y1:y2,x1:x2,:);
p1=im(:,:,1);
p2=im(:,:,2);
...
% Perform ST Analysis
[coh,VectorF]=Parallel_testStructureFiber3D1(InvV1,Jxx, Jxy, Jxz, Jyy, Jyz, Jzz,parametersDTI);
toc;


disp('Computed EV and VectorF');

% cd /hpc_atog/btho733/ABI/JZ/Fiber_DTI/Whole_Atria/small_section_from_rgboutpng_351to600/results;
cd /hpc_atog/btho733/ABI/JZ/Fiber_DTI/Whole_Atria/small_section_from_rgboutpng_Par200/results_from301;
% disp('...Saving now');
% cd /hpc_atog/btho733/ABI/JZ/Fiber_DTI/Whole_Atria/small_section_from_rgboutpng_351to600/results;
...
disp('Saved VectorF');
save('coh_s1m3rho5k25.mat','coh','-v7.3');
disp('Saved coh');
save('InvV1_s1m3rho5k25.mat','InvV1','-v7.3');
disp('Saved InvV1');
save('N_s1m3rho5k25.mat','N','-v7.3');
disp('Saved N');
tic;
...
% tic;
disp('Saving Fiber orientation estimates');
% cd /hpc_atog/btho733/ABI/JZ/Fiber_DTI/Whole_Atria/small_section_from_rgboutpng_351to600/results;
cd /hpc_atog/btho733/ABI/JZ/Fiber_DTI/Whole_Atria/small_section_from_rgboutpng_Par200/results_from301;
