...
ui <- fluidPage(
  # navbarPage(title= span(h3("Color based clustering of imbalanced classes (Quantification of atrial fibrosis)"),
   ...
   wellPanel(
    h1("Color based clustering of imbalanced classes (Quantification of atrial tissue types)")),
   tabsetPanel(
             tabPanel("Cluster Analysis",fluidRow(
                                                   #column(2,selectInput(inputId = "n",
                                         ...
                                         fluidRow(column(3,plotOutput("plot0",width="100%")),
                                                  column(3,plotOutput("plot1",width="100%")),
                                                  column(3,plotOutput("plot3",width="100%")),
                                                  column(3,plotOutput("plot2",width="auto"))),
                                         fluidRow(column(5,plotlyOutput("plot4",width="auto")),
                                                  column(4,plotlyOutput("plot5",width="auto")),
                                                  column(3,tags$img(height=400,width=350,src="Legend.png")))),
             tabPanel("Implementation details",
                      tags$h4(tags$strong("Background")),
                      fluidRow(column(12,
                                      p(style="text-align:justify","Identification and quantification of connective tissue over the atrial volume are topics of
...
identification of tissue types is highly significant in the design of such modelling studies).
            "))),
                      ...
                      fluidRow(column(12,p(style="text-align:justify","This page summarises the development of a novel color-based clustering app (see the cluster analysis tab) 
  ...
  from top to bottom by using an imaging modality called", tags$u("serial block face imaging technique (A representative slice from the stack is shown below)"),". The three types of tissue are specifically identified 
  by topical application of May-Grunwald stain", tags$strong("[Fibrosis (Collagen) is coloured purplish, Myocytes are coloured greenish and Fat cells are coloured yellowish.]") 
         ))),  
                      tags$br(),
                      fluidRow(column(6,tags$img(height=500,width=720,src="fig5_1.png")),
                               column(4,offset=1,tags$img(height=500,width=400,src="fig5_4.png"))),
                      tags$br(),
                      tags$h4(tags$strong("The problem")),
                      fluidRow(column(7,p(style="text-align:justify","In this data set, myocytes and fat cells make up the lion's share of the structure, although their distribution may vary
...
literature under the tag of ",tags$strong("class imbalance problems"),". Severely skewed distributions are notoriously known for damaging the efficacy of machine learning based workflows,
...
tags$u("The figure showing ",tags$strong("color distribution in 3 color spaces (RGB, HSB and CIELAB)"))," presents the variety of shades present in a representative slice taken from the stack.
                                      "),
                                      tags$br(),
                                      tags$h4(tags$strong("The solution")),
                                      fluidRow(column(12,p(style="text-align:justify","K-means clustering in CIELAB color space is a popular approach for color-based segmentation. 
  ...
  results delivered by this approach. ",tags$u("Refer to the figures on the right side panel to see the ",tags$strong("clustering results(k=3) produced by 3 different initialisations")),". The third set shows better segmentations compared to the other two sets.
  ...
  The solution is powered by a resampling process which produces a balanced distribution of the three dominant colors present in each slice. ", tags$u("The approach developed to ",tags$strong("filter out a*-b* plot")," is summarised in the figure below"), 
  ...
  of these estimated sectors are used to initialise k-means clustering in each slice. "))),
                               ...
                               column(5,tags$br(),fluidRow(column(10,tags$img(height=150,width=600,src="fig5_2.png"))),
                                                fluidRow(column(10,tags$img(height=150,width=600,src="fig5_3.png"))),
                                                fluidRow(column(10,tags$img(height=150,width=600,src="fig5_7.png"))),
                                      
                                      )),
                      tags$br(),
                      
                      
                      fluidRow(column(11,offset=1,tags$img(height=300,width=1300,src="fig5_5.png"))),
                      tags$br(),
                      tags$h4(tags$strong("Results")),
                      fluidRow(column(12,p(style="text-align:left","The interactive dashboard provides a set of insights which further leads to major results/observations as outlined below:  "))),
                                      fluidRow(column(12,p(style="text-align:left",tags$strong("1. Robust segmentation of tissue types from top to bottom of stack")," : This fully automatic approach enabled extraction of 3D collagen network(Fibrosis) from the stack (See figure below)."))),
                                                      fluidRow(column(12,p(style="text-align:left",tags$strong("2. Accurate quantification of the three types of tissue")," :  The visualisations provided a graphical comparison of tissue proportions at various levels - slice level, regional and overall. "))),
                                                                      fluidRow(column(12,p(style="text-align:left",tags$strong("3. Objective local analysis")," : The bumps (red rectangles) in the graph helped to locate the most fibrotic regions from Pulmonary Vein sleeves(figure A below) and lateral wall of Right Atrium(figure B below). This motivated further local analysis (See figures C and D below).   "))),
                      tags$br(),
                      fluidRow(column(5,offset=1,tags$img(height=400,width=650,src="fig5_8_1.jpg")),
                               column(5,offset=1,fluidRow(column(10,tags$img(height=200,width=400,src="Annotation.png"))),
                                      fluidRow(column(10,tags$img(height=200,width=400,src="fig5_9.png"))))),
                      tags$br(),
          )
...
server <- function(input, output, session) {
  ...
  plot(a,b,pch=15,col=h,xlab="a*",ylab="b*",font.lab=2,cex.lab=1.2)
  ...
  # height=function() session$clientData$output_plot4_height
   ...
   output$plot5 <- renderPlotly({
    ...
    mavgtiss=roll_mean(percentcsv$V3,40,fill=0)
    mavgfat=roll_mean(percentcsv$V4,40,fill=0)
    Location <- c(rep("Selected slice",3),rep("Regional",3),rep("Overall",3))
    Tissue_Type <- rep(c("Fibrosis","Myocytes","Fat"),3)
    Percentage <- c(percentcsv$V2[strtoi(input$n)],percentcsv$V3[strtoi(input$n)],percentcsv$V4[strtoi(input$n)],
                 mavgfib[strtoi(input$n)],mavgtiss[strtoi(input$n)],mavgfat[strtoi(input$n)],
                 mean(percentcsv$V2[83:1011]),mean(percentcsv$V3[83:1011]),mean(percentcsv$V4[83:1011]))
     data <- data.frame(Location,Tissue_Type,Percentage)
  
     data$Location <- factor(data$Location, levels = c('Selected slice', 'Regional', 'Overall'))
     data$Tissue_Type <- factor(data$Tissue_Type, levels = c('Fibrosis', 'Myocytes', 'Fat'))
  
    p2 <-  ggplot(data,aes(fill=Tissue_Type,y=Percentage,x=Location))+
